...
function cleanAndStart() {
    ...
    if (participantID == "") {
        ...
        displayHelpForParticipants();
    }
}

function displayHelpForParticipants() {
    var buttonNext = '<button id="help-btn" class="btn btn-md btn-info" onclick="nextDecider()">' +
        '<span class="glyphicon glyphicon-thumbs-up"></span> Entendi</button>';

    var valence = '<img width="80%;"  src=' + valenceImage + ' >';
    var arousal = '<img width="80%;" src=' + arousalImage + '>';
    var dominance = '<img width="80%;" src=' + dominanceImage + '>';

    var titleValence = '<p><b>Valência Afetiva</b></p>';
    var titleArousal = '<p><b>Ativação Fisiológica</b></p>';
    var titleDominance = '<p><b>Dominância</b></p>';

    var hasSAMScales = false;

    for (var z = 1; z <= formConfiguration.passos.length; z++) {
        if ((formConfiguration.passos[z - 1].hasOwnProperty("escalasSAM")) && (formConfiguration.passos[z - 1].escalasSAM.length > 0)) {
            hasSAMScales = true;
            break;
        }
    }

    console.log(hasSAMScales);
    
    if (hasSAMScales == true) {
        document.querySelector('#div-general-info').innerHTML += titleValence;
        document.querySelector('#div-general-info').innerHTML += "<p><b>Infeliz </b>" + valence + "<b> Feliz</b></p>";
        document.querySelector('#div-general-info').innerHTML += "<br/>";
        document.querySelector('#div-general-info').innerHTML += titleArousal;
        document.querySelector('#div-general-info').innerHTML += "<p><b>Calmo </b>" + arousal + "<b> Emocionado</b></p>";
        document.querySelector('#div-general-info').innerHTML += "<br/>";
        document.querySelector('#div-general-info').innerHTML += titleDominance;
        document.querySelector('#div-general-info').innerHTML += "<p><b>Independente </b>" + dominance + "<b> Dominado</b></p>";

        document.querySelector('#div-general-info').innerHTML += buttonNext;
    } else {
        nextDecider();
    }
}
